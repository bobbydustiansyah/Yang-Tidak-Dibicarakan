(() => {
 ...
 async function load(){if(!sb)return;const {data,error}=await sb.from("site_content").select("key,value");if(error)return;const map=Object.fromEntries((data||[]).map(x=>[x.key,x.value]));document.querySelectorAll("[data-content]").forEach(el=>{const key=el.dataset.content;if(map[key]!==undefined)el.textContent=map[key]});const photo=document.querySelector("[data-author-photo]"),img=document.querySelector("[data-author-photo-img]"),cap=document.querySelector("[data-author-photo-caption]");if(photo&&img){const url=map.about_author_image_url||"";if(url){img.src=url;photo.hidden=false;if(cap)cap.textContent=map.about_author_image_caption||"Penulis"}else{photo.hidden=true}}}
 load();
})();
