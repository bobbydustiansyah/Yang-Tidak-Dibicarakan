  ...
  async function load(){
    ...
    root.innerHTML=`
      ...
      <section class="content-layout">
        ...
        <article class="content">${item.image_url?`<figure class="content-image"><img src="${esc(item.image_url)}" alt="${esc(item.image_caption||item.title||'Foto tulisan')}">${item.image_caption?`<figcaption>${esc(item.image_caption)}</figcaption>`:''}</figure>`:''}${paragraphs(item.content)}</article>
      </section>
